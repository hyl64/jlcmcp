/**
 * bridge-server.mjs — vendored from easyeda/easyeda-api-skill
 * (https://github.com/easyeda/easyeda-api-skill, scripts/bridge-server.mjs)
 * 官方 EasyEDA WebSocket Bridge Server（单例，端口 49620-49629，服务 easyeda-bridge）。
 * 由嘉立创EDA专业版 Run API Gateway 扩展自动连接（ws://127.0.0.1:<port>/eda）。
 * 本文件按官方仓库原样提供，便于 jlceda-mcp-server 自启动；若你已通过官方
 * easyeda-api skill 运行同一服务，本文件不会重复占用端口（官方单例逻辑会退出）。
 */
/**
